...
//
// Not used here: $C061-$C063. The keyboard registers those for Open Apple,
// Closed Apple and Shift, and the joystick registers the same three for its
// buttons, and a direct IO address is answered by whichever peripheral
// registered it first. What those addresses report is therefore the one thing
// in slot 0 that does still turn on registration order, and it is a separate
// question from the one these cases are about.
...
// asked for - and taken down again whatever the case does.
struct Slot0_t {
  ...
  explicit Slot0_t(Order_t order) {
    ...
    if (order == Order_t::keyboard_first) {
      REQUIRE(peripheral_register(keyboard_get_descriptor(), 0) == 0);
      REQUIRE(peripheral_register(joystick_get_descriptor(), 0) == 0);
    } else {
      REQUIRE(peripheral_register(joystick_get_descriptor(), 0) == 0);
      REQUIRE(peripheral_register(keyboard_get_descriptor(), 0) == 0);
    }
...
TEST_CASE("Slot 0: a payload of the wrong size changes nothing") {
  for (Order_t order : both_orders) {
    ...
    JoystickConfig_t config = a_config();
    REQUIRE(peripheral_command(0, JOY_CMD_SET_CONFIG, &config,
                               sizeof(config) - 1) == peripheral_ok);
    REQUIRE(peripheral_command(0, JOY_CMD_SET_CONFIG, &config,
                               sizeof(config) + 1) == peripheral_ok);
...
TEST_CASE("Slot 0: a dispatcher says peripheral_error to the wrong size") {
  ...
  void* kbd = keyboard->init(0, &bare.host);
  void* joy = joystick->init(0, &bare.host);
  REQUIRE(kbd != nullptr);
  REQUIRE(joy != nullptr);
  ...
  CHECK(keyboard->command(kbd, keyboard_cmd_set_rocker, &byte, 0) ==
        peripheral_error);
  CHECK(keyboard->command(kbd, keyboard_cmd_set_rocker, &byte, 2) ==
        peripheral_error);
  ...
  JoystickConfig_t config = a_config();
  CHECK(joystick->command(joy, JOY_CMD_SET_CONFIG, &config, sizeof(config)) ==
        peripheral_ok);
  CHECK(joystick->command(joy, JOY_CMD_SET_CONFIG, &config,
                          sizeof(config) - 1) == peripheral_error);
  CHECK(joystick->command(joy, JOY_CMD_SET_CONFIG, &config,
                          sizeof(config) + 1) == peripheral_error);
  CHECK(joystick->command(joy, JOY_CMD_SET_CONFIG, nullptr, sizeof(config)) ==
        peripheral_error);
...
TEST_CASE("Slot 0: a command can name the peripheral it is for") {
  for (Order_t order : both_orders) {
    ...
    // A peripheral that is not in the slot, and an id too long to hold, are
    // both refused rather than delivered to whoever happens to be there.
    CHECK(peripheral_command_by_id(0, "linapple.disk_II",
                                   keyboard_cmd_set_rocker, &on,
                                   sizeof(on)) == peripheral_error);
    CHECK(peripheral_command_by_id(
              0, "linapple.a-name-that-does-not-fit-in-the-queue",
              keyboard_cmd_set_rocker, &on, sizeof(on)) == peripheral_error);
    CHECK(peripheral_command_by_id(0, nullptr, keyboard_cmd_set_rocker, &on,
                                   sizeof(on)) == peripheral_error);
