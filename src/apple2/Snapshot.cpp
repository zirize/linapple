...
auto snapshot_serialize(ApplewinSnapshot_t* snapshot) -> void {
  ...
  *snapshot = ApplewinSnapshot_t{};
  ...
  {
    // By name, not by position: slot 0 holds the keyboard and the speaker as
    // well, and peripheral_save_state() serialises whichever of the three
    // registered first. The keyboard below and the speaker in the slot loop
